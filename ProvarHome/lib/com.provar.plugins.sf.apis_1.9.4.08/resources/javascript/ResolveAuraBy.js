...
function resolveAuraBy(componentXPath, searchContext, multiple) {
    ...
    if (componentXPath.indexOf("@*[name()") === -1) {
        ...
        attValue = cleanValue(attValue);
         if (isForceModal) {
            componentXPath = "//*[name() = 'force:modal']//*[name() = '"+ tagName +"'][@*[name() = '" + att + "'] =" + attValue;
         } else {
         ...
         if (isForceModal) {
            componentXPath = "//*[name() = 'force:modal']//*[name() = '"+ tagName +"']" + attribute;
         } else {
    ...
    if(resultElem && qualifier && qualifier.length === 2){
        ...
        while (resultElem) {
            ...
            if(htmlElem && htmlElem.firstChild && htmlElem.firstChild.tagName){
            		var htmlElemTagName=htmlElem.firstChild.tagName;
            		if(htmlElemTagName === "LIGHTNING-INPUT" || htmlElemTagName === "LIGHTNING-PICKLIST" || htmlElemTagName === "LIGHTNING-DATEPICKER" ||
            				((htmlElemTagName === 'DIV'  || htmlElemTagName === 'LABEL') && htmlElem.firstChild.getAttribute('lightning-input_input') ==="")) {
...
function getFirstHtmlInputElement(htmlElem) { 
	...
	var elem = htmlElem.querySelector("input[lightning-input_input].slds-input, input[lightning-input_input][type=checkbox]");
    return elem;
}
